  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 px-6 py-4 flex justify-between items-center bg-background/80 backdrop-blur-md border-b border-border transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
      ...
      <div className="flex items-center gap-4">
        ...
        <Link to="/contact" className="bg-foreground text-background px-5 py-2.5 rounded-full text-sm font-semibold hover:opacity-80 transition-all duration-300">
          Book a Lesson
        </Link>
        <button className="lg:hidden text-foreground transition-colors duration-300">
          <Menu size={24} />
        </button>
      </div>
    </nav>
