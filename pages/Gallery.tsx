      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-12 overflow-x-auto pb-4">
          {["All", "Action", "Social", "Sunsets", "Clubhouse"].map((cat, i) => (
            <button key={i} className={`px-6 py-2 rounded-full border text-sm font-medium whitespace-nowrap transition-colors ${i === 0 ? 'bg-foreground text-background border-foreground' : 'border-foreground/20 text-muted hover:text-foreground'}`}>
              {cat}
            </button>
          ))}
        </div>
